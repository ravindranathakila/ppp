...
function(weights.data, just.removed, remaining, ...){
        ...
        if(length(just.removed)==0){
            cat("No species removed, no need to update weights\n")
        ...
        # NB: this column is only called 'Ts' by the aggregate function, it's
        # really 'As'
        ...
        Ag <- aggregate(Tg ~ genus + family, 
            weights.data[to.update.fam, ], FUN = sum)
        ...
        Af <- aggregate(Tf ~ family + order, 
            weights.data[to.update.ord, ], FUN = sum)
        ...
        # 2.1.) Find those species_ids that were in the same genus as a 
        # removed species.  
        ...
        # (this is takes into account more species than is necessary but 
        # it is concise code)
        last.spp <- unlist(subset(weights.data, 
            subset = (Bs - As) == 1, select = species_id))
        
        # 3.) Intersect the remaining list and the affected list
        # Those species that are in the same order as the removed 
        # species AND are the last in their genus.  
        affected <- intersect(same.ord, last.spp)
        
        # Include those species that are in the same genera as the 
        # removed species

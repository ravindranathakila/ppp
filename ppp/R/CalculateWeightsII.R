...
function(weights.data, affected, weighting = 1, multiplier = 1, ...){
    ...
    if(length(affected) == 0){
        cat("The removed species does not affect W, no need to recalculate weights\n")
        return(weights.data)}
    ...
    for(i in affected){
        ...
        affected.gen <- subset(weights.data, subset = species_id == i, 
            select = genus, drop = TRUE)
        w.sub.spp <- subset(weights.data, genus == affected.gen)
        
        # Set species i to 'threatened'; that is Ts <- 1
        #w.sub.spp[w.sub.spp$species_id == i, "Ts"] <- 1
        index.spp <- with(w.sub.spp, which(species_id == i))
        w.sub.spp <- within(w.sub.spp, Ts[index.spp] <- 1)
        
        # Recalculate As as the sum of Ts in the genus.
        w.sub.spp <- within(w.sub.spp, 
            As[index.spp] <- aggregate(Ts ~ genus, w.sub.spp, FUN = sum)$Ts)
        
        # Set Tg to 1 if all the species in the genus are threatened.
        if(with(w.sub.spp[index.spp, ], As/Bs == 1)){
            w.sub.spp[index.spp, "Tg"] <- 1
        }
        w.sub <- w.sub.spp
        ...
        if(w.sub.spp[index.spp, "Tg"] == 1){
            ...
            affected.fam <- subset(weights.data, subset = species_id == i,
                select = family, drop = TRUE)
            ...
            if(with(w.sub.gen[index.gen, ], Ag/Bg == 1)){
                w.sub.gen[index.gen,"Tf"] <- 1
            }
            ...
            if(w.sub.gen[index.gen, "Tf"] == 1){
                ...
                affected.ord <- subset(weights.data, subset = species_id == i, 
                    select = order, drop = TRUE)
                w.sub.fam <- subset(weights.data, 
                    subset = order == affected.ord)
